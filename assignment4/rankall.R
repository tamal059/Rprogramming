rankall <- function(outcome,num="best"){
  ...
  stateList <-unique(dataset$State)
  len<-length(stateList)
  output <-data.frame("hospital"=numeric(len),"state"=numeric(len))
  
  for (i in seq_along(stateList)){
    hospitalName<-rankhospital(dataset,stateList[i],outcome,num)
    if(identical(hospitalName,"Invalid Num Input")){
      hospitalName<-"<NA>"
    }
    output$hospital[i]<-hospitalName  
    output$state[i]<-stateList[i]
  }
